...
use std::path::Path;
use wgpu;
use crate::loaders::ShaderLoaderTrait;
use crate::loaders::ShaderLoader;
use crate::loaders::PipelineGenerator;
use crate::loaders::PipelineGeneratorTrait;
...
use crate::graphics::GlobalLayouts;
...
"#;
    ...
    fn scan_and_generate_pipelines(
        ...
        for entry in entries {
            ...
            if path.extension().and_then(|s| s.to_str()) == Some("wgsl") {
                ...
                // 加载并创建 Pipeline
                let pipeline = self.create_pipeline(device, global_layouts, &relative_path, format, depth_format)?;
    ...
    fn create_pipeline(
        ...
        // Use GlobalLayouts for Group 0 and Group 1
        let frame_layout = &global_layouts.frame_layout;
        let pass_layout = &global_layouts.pass_layout;
        // Group 2 (Material) is not known here, so we assume custom shaders without material config
        // either don't use Group 2, or rely on derived layout (which we can't mix easily).
        // Let's pass Group 0 and 1. If the shader needs Group 2, this will fail or require a dummy layout.
        // For now, let's assume simple custom shaders.
        self.create_gpu_shader(device, shader_path, format, depth_format, &[frame_layout, pass_layout], &PipelineState::default())
